<script>
  ...
  let armor = $derived(equipment && equipment.get("armor"));
  let weapon = $derived(equipment && equipment.get("weapon"));
...
<div>
  ...
  <ul>
    {#if armor != null}
      <li class="item" onclick={() => selectItem(armor)} on:keypress={() => selectItem(armor)}>
    ...
    {/if}
    {#if weapon != null}
      <li class="item" onclick={() => selectItem(weapon)} on:keypress={() => selectItem(armor)}>
        <div
